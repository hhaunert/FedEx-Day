...
function ExampleModal({ type, data, onClose }: { type: ExampleKey; data: unknown; onClose: () => void }) {
...
export default function FeatureExamples({ clueReport, researchTrail, storyPath }: FeatureExamplesProps) {
  const [activeExample, setActiveExample] = useState<ExampleKey | null>(null)

  const dataMap: Record<ExampleKey, unknown> = {
    'clue-report': clueReport,
    'research-trail': researchTrail,
...
function ExampleModal({ type, onClose }: { type: ExampleKey; onClose: () => void }) {
  const { title, data } = EXAMPLES[type]
  return (
    <div
      className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4"
      onClick={onClose}
