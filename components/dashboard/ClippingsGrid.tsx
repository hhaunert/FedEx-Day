...
export default function ClippingsGrid({ clippings: initial }: { clippings: Clipping[] }) {
  ...
  if (clippings.length === 0) {
    return (
      <div className="card p-6 sm:p-16 text-center">
      ...
      {clippings.map((clipping) => (
        <div key={clipping.id} className="card overflow-hidden group relative">
          ...
          <button
            ...
            className="absolute top-2 right-2 z-10 w-9 h-9 bg-white/90 hover:bg-red-50 border border-brand-gray-border hover:border-brand-red rounded-full flex items-center justify-center opacity-100 sm:opacity-0 sm:group-hover:opacity-100 transition-all duration-150"
            title="Delete"
          >
            <svg className="w-4 h-4 text-brand-gray-mid hover:text-brand-red" fill="none" stroke="currentColor" viewBox="0 0 24 24">
